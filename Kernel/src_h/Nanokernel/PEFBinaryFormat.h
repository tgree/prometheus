...
struct PEFContainerHeader {
	UInt32 							tag1;						/* Must contain 'Joy!'. */
	UInt32 							tag2;						/* Must contain 'peff'.  (Yes, with two 'f's.) */
	UInt32 							architecture;				/* The ISA for code sections.  Constants in CodeFragments.h. */
...
struct PEFSectionHeader {
	Int32 							nameOffset;					/* Offset of name within the section name table, -1 => none. */
...
struct PEFLoaderInfoHeader {
	Int32 							mainSection;				/* Section containing the main symbol, -1 => none. */
	UInt32 							mainOffset;					/* Offset of main symbol. */
	Int32 							initSection;				/* Section containing the init routine's TVector, -1 => none. */
	UInt32 							initOffset;					/* Offset of the init routine's TVector. */
	Int32 							termSection;				/* Section containing the term routine's TVector, -1 => none. */
...
/* 																								*/
/*		UInt8	PEFComputeHashTableExponent	( Int32	exportCount )							*/
/*		{																						*/
/*			Int32	exponent;																	*/
/* 																								*/
/*			const Int32	kExponentLimit		= 16;	// Arbitrary, but must not exceed 30.	*/
/*			const Int32	kAverageChainLimit	= 10;	// Arbitrary, for space/time tradeoff.	*/
/* 																								*/
...
/*			Int32	hashValue	= 0;		// ! Signed to match old published algorithm.				*/
/*			UInt32	length		= 0;																	*/
/*			UInt32	limit;																				*/
/*			UInt32	result;																				*/
/*			UInt8	currChar;																			*/
...
struct PEFExportedSymbol {																/* ! This structure is 10 bytes long and arrays are packed. */
	...
	Int16 							sectionIndex;				/* The index of the section, or pseudo-section, for the symbol. */
};
